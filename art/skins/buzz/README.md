# buzz — a recolour, baked

Honeycomb: warm orange with hexagonal cells over the body and a darker comb on the belly.
From `assets/source/skins/BUZZ/BUZZ.glb`, the BOG sculpt retextured by Tripo on
...
Baked by `python tools/bake_skin.py BUZZ` (which `extract_skins.py` calls on its
