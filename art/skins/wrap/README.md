# wrap — a recolour, baked

A mummy: wound head to toe in beige linen bandage.
From `assets/source/skins/WRAP/WRAP.glb`, the BOG sculpt retextured by Tripo on
...
Baked by `python tools/bake_skin.py WRAP` (which `extract_skins.py` calls on its
