...
```
bash tools/smoke_test.sh        # 116 checks, ~6 minutes, finds Godot by itself
bash tools/net_test.sh          # two processes, one socket; ~45 s, run by hand
```

`smoke_test.sh` is the gate and it passes, 116 of 116. `net_test.sh` is kept out
...
| tool | proves |
|---|---|
| `tools/smoke_test.sh` | **the gate** — import, and one hundred and sixteen checks |
