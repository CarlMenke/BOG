...
## Drives the Bog's skeleton from the state of the `Bog` body above it.
##
...
## (D-064). What they are not is two *timings* — `BogCombat` fires exactly one of
...
## reaches the same pose from the same charge, and a Bog that is half drawn is
...
## on the locomotion plane would leave a Bog spinning from the waist up over a
...
##
## **`carry` is the one node in this graph that asks which weapon a Bog has**
...
## whole poses — a landing absorb that flailed the arms of a Bog holding two
## metres of blade would be the prop, not the pose, that the eye followed.
##
## **`stand` is a plane and not a line, which is the whole of D-066.** A Bog
## aiming holds its facing at the crosshair (`Bog._face_view`) and moves
## wherever the keys say, so "how fast" stopped being enough to pick a pose
## with: a Bog running sideways at 5.4 m/s and a Bog running forward at 5.4 m/s
...
## speed`) in a custom timeline, so a Bog travelling at exactly one of the three
...
## renames or drops one would otherwise show up as a Bog that simply never
...
const REQUIRED_CLIPS: Array[String] = [
	...
	# is an error on the frame the animator is built, on every Bog, instead of a
	# Bog carrying a great sword in a boxer's guard on two players in three.
	#
	...
	# Bog's own: there is no third clip that a rebuild could drop without the
...
## vertical motion differently (`VERTICAL_RISE_KEPT` in `tools/build_bog.py`),
...
## that. `Bog.SLIDE_DURATION` (1.0 s) ends the physical slide at 1.10 s of clip,
...
## `tools/build_bog.py`), and from 1.48 to ~1.60 the skin is 0.15-0.25 m under
...
## so a window that opened any earlier would be a Bog sprinting on the spot into
...
## back toward the body. `tools/build_bog.py` prints that same 1.567 at the end
...
## seconds. `BogCombat` reads this, and it is derived rather than typed so that
...
##
## The 0.567 s before it is a Bog taking an arrow out of a quiver, and it cannot
## be in the charge however good it looks. A bow is **carried**: charge zero has
## to be a nocked bow at brace, or a snap shot fires an arrow the Bog is still
...
## starts at about 0.36 and is a recovery a Bog still holding a bow has no use
## for.
...
## (`Bog.draw_fraction`). Both are correct on the seven Bogs you are watching
...
##   leaving there comes out of the Bog rather than out of the hand.
...
## approach `lock_root_motion` would have the Bog run on the spot; the cast
...
## `Bog.SPIN_ADVANCE` is a distance the animation no longer covers, which is the
...
## plane while the Bog is still moving, which is the right way round: a swing
## that ended by planting both feet would stop a Bog that is supposed to be
...
##
## `tools/build_bog.py` prints the same 1.067 at the end of every build —
##
...
## `BogCombat` reads this, and it is derived through the rate rather than typed
...
##
## The other half of `Bog.SPIN_ADVANCE`: the body covers those 1.712 m over
...
## for, and it is what `BogCombat.sword_cycle()` is measured against.
...
##
## `Drink` is `6_Utility/Drinking.fbx`, 6.117 s, and most of it is a Bog
...
## are measured**. `tools/build_bog.py` tracks the *drinking* hand — the left
...
## host who drags the channel mid-match does not leave one Bog drinking at the
...
##
## It is `Bog.ROLL_MIN_AIRTIME` rather than its own number because the body uses
...
## pose and the prop's own lever have to agree about whether this Bog is carrying
...
## `Loose` one-shot after that the Bog is still holding an archer's pose out of
...
## The torso modifier, installed in `_ready` rather than declared in `bog.tscn`
## because the skeleton it hangs off lives inside the imported `art/generated/
## bog.glb` and reaching into an instanced scene from a `.tscn` means marking
...
## Whether this Bog was drawing a bow last frame. Kept for the same reason
## `_sliding` is: the interesting thing about a draw is the frame it *ends*.
var _drawing: bool = false

## The current airtime. `_airtime_open` is false while the Bog is standing on
...
## from the body in `_ready`, so a Bog that spawns into a match already several
...
func _ready() -> void:
	...
	if not missing.is_empty():
		push_error("BogAnimator: art/generated/bog.glb is missing clips: %s"
			% ", ".join(missing))
		return
	...
	if skeleton == null:
		push_warning("BogAnimator: no Skeleton3D under this Bog; the torso "
			+ "will not aim")
	else:
	...
	# The carry pose, chosen once (D-070). `Bog.weapon` is fixed before the body
	# exists and cannot change while it lives, so this is the only place the
	# graph ever asks which weapon a Bog has — and `set_carry_pose` is public
	...
	# A Bog that dies in mid-air is put back on the ground somewhere else, and it
	...
	# Standing, whatever the body says. A Bog is spawned onto the ground, and a
	...
	# `tools/combat_range.tscn` and every Bog in the lobby splayed out mid-leap.
...
func _find_skeleton_track_prefix(player: AnimationPlayer) -> String:
	...
	push_warning("BogAnimator: no skeleton tracks found; throw will play full-body")
	return ""
...
func _build_graph(player: AnimationPlayer) -> AnimationNodeBlendTree:
	...
	crouch.max_space = Bog.CROUCH_SPEED
	crouch.sync = true
	crouch.add_blend_point(_cycle(player, "CrouchIdle", 0.0, 0.0), 0.0, -1, "still")
	crouch.add_blend_point(
		_cycle(player, "CrouchWalk", Bog.CROUCH_SPEED, Bog.AUTHORED_CROUCH_WALK),
		Bog.CROUCH_SPEED, -1, "walk")
	...
	# (D-029), so a filtered version of this clip would turn a Bog's chest
	...
	# drink is running — `BogCombat` gates both ways — so the only overlap there
	# can be is the drink's own 0.18 s fade-out, which a cancelled channel leaves
	# running while the Bog is free to act again on the same frame. In that
	...
	# be a Bog holding a bottle in a stance built round a longbow. It is the
	...
	# the one whose weapon is in the hand on the frame the tie happens. A Bog
	# that stops being the Elder has a spear again immediately; a Bog that starts
	...
	# arbitrary even though only one of the two is ever fired per click: a Bog
	...
	# and nothing can start any of them during a swing — `BogCombat` gates both
...
##
## Positions are the Bog's velocity **in its own frame**, x to its right and y
...
func _locomotion(player: AnimationPlayer) -> AnimationNodeBlendSpace2D:
	...
	stand.min_space = Vector2(-Bog.RUN_SPEED, -Bog.RUN_SPEED)
	stand.max_space = Vector2(Bog.RUN_SPEED, Bog.RUN_SPEED)
	stand.x_label = "right"
	...
	stand.add_blend_point(_cycle(player, "Walk", walk, Bog.AUTHORED_WALK),
		Vector2(0.0, walk), -1, "walk")
	stand.add_blend_point(_cycle(player, "Run", run, Bog.AUTHORED_RUN),
		Vector2(0.0, run), -1, "run")
	stand.add_blend_point(_cycle(player, "WalkBack", walk, Bog.AUTHORED_WALK_BACK),
		Vector2(0.0, -walk), -1, "walk back")
	stand.add_blend_point(_cycle(player, "RunBack", run, Bog.AUTHORED_RUN_BACK),
		Vector2(0.0, -run), -1, "run back")
	stand.add_blend_point(
		_cycle(player, "StrafeWalkRight", walk, Bog.AUTHORED_STRAFE_WALK_RIGHT),
		Vector2(walk, 0.0), -1, "walk right")
	stand.add_blend_point(
		_cycle(player, "StrafeRight", run, Bog.AUTHORED_STRAFE_RIGHT),
		Vector2(run, 0.0), -1, "run right")
	stand.add_blend_point(
		_cycle(player, "StrafeWalkLeft", walk, Bog.AUTHORED_STRAFE_WALK_LEFT),
		Vector2(-walk, 0.0), -1, "walk left")
	stand.add_blend_point(
		_cycle(player, "StrafeLeft", run, Bog.AUTHORED_STRAFE_LEFT),
...
## and `WalkBack` as much as in `Idle`. A Bog standing still is simply in the
## clip it would have been in anyway.
##
## `xfade_time` is zero and `allow_transition_to_self` is off for the same
## reason: `Bog.weapon` is set before the body exists and cannot change while it
## lives (D-069), so this node is written exactly once per Bog, in `_ready`, and
## a cross-fade would be a fade out of a pose nothing had ever been in. The
## exception is the lobby ring, where `BogBackdrop` does change `Bog.weapon` on a
## standing Bog — and there a hard cut is right as well, because the strip is a
...
## frame the first time a Bog in the ring was arrowed onto.
func _carry_pick() -> AnimationNodeTransition:
	...
	for i in Loadout.CARRY_CLIPS.size():
		...
		# and the ring's Bogs would all breathe in step with the arrow keys.
...
## under a Bog at full draw — which is the whole of "every attack works in the
## air" for this weapon and costs nothing (D-065).
##
## `sync` comes from `_blend2` and matters here for its usual reason: the branch
## underneath has to keep running while the draw is at full weight, or a Bog
...
## drink in the air" (D-067): the legs keep the air-arc pose under a Bog raising
...
func _process(delta: float) -> void:
	...
	set(P_CROUCH_SPEED, clampf(flat.length(), 0.0, Bog.CROUCH_SPEED))
	...
	# The draw, every frame, on every peer's copy of every Bog — which is the
	# point of it. `draw_fraction()` answers off the replicated float on a remote
	# Bog and off the local one on your own, exactly as `is_crouching` does, so
	...
	# rather than from `BogCombat._tick_draw` beside the string for a reason
	...
	# (D-066). It runs on every peer's copy of every Bog, so a remote archer
	# brings its bow up out of the carry exactly as the local one does.
	# **One number for the pose and for the prop's own lever** (D-070). A Bog is
	...
	#
	# It runs on every peer's copy of every Bog, so a remote archer brings its
...
## (D-066): x to the Bog's right, y forward.
##
## Off `body_yaw` and not off the node's transform: `_model_root` is turned by
## the same yaw but the Bog mesh is authored facing +Z and counter-rotated 180°
## inside `bog.tscn`, so reading a basis here would be reading that
## compensation as well. `facing()` is the accessor that already means "the way
## the Bog looks" on both sides of that, and it answers off the replicated yaw
## on a remote Bog — which is the whole reason the sideways pose arrives on
...
## wrong ring everywhere except on an axis — a Bog walking diagonally at 2.3 m/s
...
## stays.
##
## What flips it is the one download named in `build_bog.py`'s strafe block. The
## day the walk poles are laterals too, this function should return `move`
## unrescaled and the harness should say so.
##
## The component clamp that survives is for the Elder: a boosted Bog runs at
## 7.3 m/s (D-040) and there is no clip out there, so it sits on the run ring
## and plays the run cycles slightly slow, which is the trade `Bog.target_speed`
...
## makes a fall work with no extra clip. A Bog that walks off a ledge has vy of
...
## So a grounded Bog holds the about-to-land pose instead, which is also the pose
...
func _scrub_air() -> void:
	...
	# value is a physics tick fresher than the copy in `velocity`. See `Bog`.
	var vy := _body.vertical_speed()
	# `jump_velocity()` rather than the constant, because the Elder's boost
	# multiplies it (D-040). Against a fixed 9.0 a Bog that left the ground at
	...
	# `elder_scale` is: the robe is on every peer's copy of the Bog, so a remote
...
## measured against — `Bog._dive` has already added DIVE_UP_VELOCITY to the
...
## is not yet true at the spawn point, and believing it would put the Bog into
...
## Bog's bow snaps on the same frame as the string it is attached to. The draw
...
## "remote Bogs see it because a number changed, not because a message arrived",
...
## Fire the throw animation. Called on every peer, so remote Bogs visibly throw,
...
##
## The default is the spear's, so every existing caller is unchanged and a Bog
...
## and made by the same line of `BogCombat._play_windup` on every peer (D-064).
##
...
## (D-067). Called on every peer by `BogCombat._do_drink_potion`, the same way
...
## an arm teleporting back to the Bog's side. The fade is DRINK_FADE_OUT, the
...
## `BogCombat._begin_swing`, the same way `_play_windup` is, so a remote Bog
...
## this Bog is in the middle of an attack it should be kept facing the crosshair
...
## way down is a Bog whose shot has already gone. So it asks the body, which is
...
## commits its direction at the click and `Bog._handle_movement` then drives the
## body along it for 1.867 s. A yaw that went on tracking the view would slide a
## Bog one way while it was drawn going another, and would hand a player a way to
## re-point an advance they had already paid for. `Bog._face` refuses the turn
...
## to keep pointed at a crosshair: a Bog drinking is not aiming at anybody, and
...
## `BogAim`, which is the only thing that asks.
##
...
## Is there actually a weapon in this Bog's hands (D-070)?
##
## **The carry pose is a weapon pose, so a Bog with no weapon must not be in
## one.** Four things take a Bog's weapon away and all four matter here: an Elder
...
## until the shaft grows back. A Bog standing in a two-handed guard with nothing
...
##
## Asked of the **hands** and not of `BogCombat`'s gates, which is the point.
...
## copy of it, and it is already true on the seven Bogs you are watching.
func _armed() -> bool:
	var gear := _body.held_gear
	return gear != null 		and (gear.is_carried() or gear.has_bow() or gear.has_sword())


## Point the carry layer at this Bog's own weapon (D-070).
##
## Called from `_ready`, and by `BogCombat.refresh_hand` for the one case where
## the answer can change under a Bog that already exists: `BogBackdrop` moves
## `Bog.weapon` on a ring Bog as the lobby's caret moves, and the pose has to
