...
## Turns the Gub's torso so the bow points where the crosshair points (D-066).
##
...
## pelvis. D-065 measured the composed pose at **91° off the Gub's own facing**
...
## until now a Gub aiming at a roof aimed a level bow at it.
##
...
##
## The second is that one bone is the wrong number of bones. A Gub is a blob
...
## the race `SkeletonModifier3D` exists to remove, and `gub.tscn` puts the
...
##
## **It does not move where an arrow goes.** `GubCombat._throw_origin()` is
## built from `global_position`, `eye_height()` and `body_yaw`, and the
## direction is `GubCamera.aim_ray()` — three things this class never touches,
...
##
## ## One code path for eight Gubs
##
## Everything read here is either local-or-replicated already (`Gub.aim_pitch`
## is `draw_fraction`'s twin) or comes off the animator's own blend weight,
## which is itself driven by the replicated draw. A Gub aiming at the sky is
...
## spine and hardly at all in the lumbar, and the Gub's own proportions say the
...
## How far the composed bow points off the Gub's own facing, in degrees, with
...
## than typed here. `GubCamera` already clamps the view to -69°..+54°, and a
...
const PITCH_MIN := GubCamera.PITCH_MIN
const PITCH_MAX := GubCamera.PITCH_MAX

var _body: Gub
var _animator: GubAnimator
...
## drive it. Called from `GubAnimator._ready`, which is the node that already
## owns what this skeleton is doing.
static func install(skeleton: Skeleton3D, body: Gub, animator: GubAnimator) -> GubAim:
	var aim := GubAim.new()
	aim.name = "Aim"
	aim._body = body
...
func _ready() -> void:
	...
	if skeleton == null:
		push_error("GubAim expects to be a child of a Skeleton3D")
		return
	...
	if not missing.is_empty():
		push_warning("GubAim: rig has no %s; the torso will not aim"
			% ", ".join(missing))
...
## same pair. `Model/gub` is turned 180° inside `gub.tscn` (the mesh is authored
...
func _process_modification() -> void:
	...
	# frame for every Gub in the match to say so is work for nobody.
	if weight <= 0.001:
		return
