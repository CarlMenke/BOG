...
## it is the wrong shape for a limit. 4 m/s is a little faster than a Gub runs,
...
## A wall arriving behind the Gub is arriving at a known rate — the rate the
...
## taking another 0.26 m off it for a warning puts the lens inside the Gub's own
...
##
## The shoulder is an angle, not a length (D-059). It is there to keep the Gub
...
## 0.62 m while the boom came in, it swung the picture sideways: the Gub slid
...
func _place_camera(delta: float) -> void:
	...
	# Gub's head is a test of the scenery 3.6 m from the thing it is placing.
...
## rather than collapsing the camera into the Gub's skull.
##
