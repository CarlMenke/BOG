...
func _run_capture() -> void:
	...
	_check("the card stays in the vault", MatchState.capture_state(MatchState.LETTER_G),
		"home")
	_check("and the vault is the team's", MatchState.banked_team_of(MatchState.LETTER_G), 0)
	...
	MatchState.claim_pickup(1, _capture_card(MatchState.LETTER_G))
	_check("a team cannot pick its own banked card back up",
		MatchState.is_holding_letter(1), false)
	_check("and it still has the letter", MatchState.team_letters(0), MatchState.LETTER_G)
	...
	# timer exists at all.
	MatchState.claim_pickup(901, _capture_card(MatchState.LETTER_G))
	...
	_check("and it is in nobody's vault",
		MatchState.banked_team_of(MatchState.LETTER_G), MatchConfig.TEAM_NONE)
