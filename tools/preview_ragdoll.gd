extends Node3D
## Fixed-camera ragdoll test: a Bog is killed on a known frame and the corpse is
...
func _ready() -> void:
	...
	# along -Z, and from 10 m away (where this camera first sat) a 1.8 m Bog is
	...
	# the height of a standing Bog's own eyes — because the thing being judged
